...
public:
	...
	CALL_VIRTUAL(8U,	GetAllClasses(),			c_client_class* (__thiscall*)(void*));
...
public:
	...
	virtual void		_1() {}
	// Effects:
	virtual void		Smoke(const vec3d_t& Origin, int Index, float Scale, float Framerate) = 0;
	virtual void		Sparks(const vec3d_t& Position, int Magnitude = 1, int TrailLength = 1, const vec3d_t* Direction = NULL) = 0;
	virtual void		Dust(const vec3d_t& Position, const vec3d_t& Direction, float Size, float Speed) = 0;
	virtual void		MuzzleFlash(const vec3d_t& Origin, const ang_t& Angles, float Scale, int Type) = 0;
	virtual void		MetalSparks(const vec3d_t& Position, const vec3d_t& Direction) = 0;
	virtual void		EnergySplash(const vec3d_t& Position, const vec3d_t& Direction, bool Explosive = false) = 0;
	virtual void		Ricochet(const vec3d_t& Position, const vec3d_t& Direction) = 0;
...
public:
	...
	const c_client_class* m_pClientClass;
	void*				m_pData;
