...
bool adbg_BeingDebuggedPEB(void)
{
    ...
    if (found) {
        DBG_MSG(DBG_BEINGEBUGGEDPEB, XOR("[Anti-Debug] Caught by BeingDebuggedPEB"));
        return true;
    }
...
bool adbg_CheckRemoteDebuggerPresent(void)
{
    ...
    if (found) {
        DBG_MSG(DBG_CHECKREMOTEDEBUGGERPRESENT, XOR("[Anti-Debug] Caught by CheckRemoteDebuggerPresent"));
        return true;
    }
...
bool adbg_CheckWindowName(void)
{
    BOOL found = FALSE;
    HANDLE hWindow = NULL;
    const char* WindowNameOlly = "OllyDbg - [CPU]";
    const char* WindowNameImmunity = "Immunity Debugger - [CPU]";
    ...
    if (found)
    {
        DBG_MSG(DBG_FINDWINDOW, XOR("[Anti-Debug] Caught by CheckWindowName"));
        return true;
    }
        ...
        do
        {
            for (const wchar_t *debugger : debuggersFilename)
            {
                ...
                if (_wcsicmp(debugger, processName) == 0) {
                    DBG_MSG(DBG_PROCESSFILENAME, XOR("[Anti-Debug] Caught by ProcessFileName"));
                    return true;
                }
            }
...
bool adbg_CheckWindowClassName(void)
{
    BOOL found = FALSE;
    HANDLE hWindow = NULL;
    const char* WindowClassNameOlly = XOR("OLLYDBG");		// OllyDbg
    const char* WindowClassNameImmunity = XOR("ID");			// Immunity Debugger
    ...
    if (found)
    {
        DBG_MSG(DBG_FINDWINDOW, XOR("[Anti-Debug] Caught by FindWindow (ClassName)!"));
        return true;
    }
...
bool adbg_IsDebuggerPresent(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_ISDEBUGGERPRESENT, XOR("[Anti-Debug] Caught by IsDebuggerPresent!"));
        return true;
    }
...
bool adbg_NtGlobalFlagPEB(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_NTGLOBALFLAGPEB, XOR("[Anti-Debug] Caught by NtGlobalFlag PEB check!"));
        return true;
    }
...
bool adbg_NtQueryInformationProcess(void)
{
    ...
    if (NT_SUCCESS(status)) {
        ...
        if (pPeb)
        {
            ...
            {
                DBG_MSG(DBG_NTQUERYINFORMATIONPROCESS, XOR("[Anti-Debug] Caught by NtQueryInformationProcess (ProcessDebugPort)!"));
                return true;
            }
        }
    }
...
bool adbg_NtSetInformationThread(void)
{
    ...
    NtSetInformationThread = (_NtSetInformationThread)GetProcAddress(hNtdll, XOR("NtSetInformationThread"));
...
bool adbg_DebugActiveProcess(const char* cpid)
{
    ...
    CreateMutex(NULL, FALSE, XOR("antidbg"));
    ...
    snprintf(cmdline, sizeof(cmdline), XOR("%ws %d"), szPath, pid);
    ...
    if (found)
    {
        DBG_MSG(DBG_DEBUGACTIVEPROCESS, XOR("[Anti-Debug] Caught by DebugActiveProcess!"));
        return true;
    }
...
bool adbg_RDTSC(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_RDTSC, XOR("[Anti-Debug] Caught by RDTSC!"));
        return true;
    }
...
bool adbg_QueryPerformanceCounter(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_QUERYPERFORMANCECOUNTER, XOR("[Anti-Debug] Caught by QueryPerformanceCounter!"));
        return true;
    }
...
bool adbg_GetTickCount(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_GETTICKCOUNT, XOR("[Anti-Debug] Caught by GetTickCount!"));
        return true;
    }
...
bool adbg_HardwareDebugRegisters(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_HARDWAREDEBUGREGISTERS, XOR("[Anti-Debug] Caught by a Hardware Debug Register Check!"));
        return true;
    }
...
bool adbg_MovSS(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_MOVSS, XOR("[Anti-Debug] Caught by a MOV SS Single Step Check!"));
        return true;
    }
...
bool adbg_CloseHandleException(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_CLOSEHANDLEEXCEPTION, XOR("[Anti-Debug] Caught by an CloseHandle exception!"));
        return true;
    }
...
bool adbg_SingleStepException(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_SINGLESTEPEXCEPTION, XOR("[Anti-Debug] Caught by a Single Step Exception!"));
        return true;
    }
...
bool adbg_Int3(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_INT3CC, XOR("[Anti-Debug] Caught by a rogue INT 3!"));
        return true;
    }
...
bool adbg_PrefixHop(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_PREFIXHOP, XOR("[Anti-Debug] Caught by a Prefix Hop!"));
        return true;
    }
...
bool adbg_Int2D(void)
{
    ...
    if (found)
    {
        DBG_MSG(DBG_NONE, XOR("[Anti-Debug] Caught by a rogue INT 2D!"));
        return true;
    }
...
void adbg_CrashOllyDbg(void)
{
    ...
    __try {
        OutputDebugString(XOR(TEXT("%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s")));
    }
    __except (EXCEPTION_EXECUTE_HANDLER) { ; }
}
